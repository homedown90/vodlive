	...
	$.fn.select2tree = function(options) {
		...
		opts.templateResult = function(data, container) {
			if(data.element) {
				...
				if($element.attr("parent")||(data.parent &&( data.parent !== 0 && data.parent !== '0'))) {
					$(container).attr("parent", $element.attr("parent")|| data.parent);
				}
				return $wrapper;
			} else {
				return data.text;
	...
	function open() {
		setTimeout(function() {
			...
			$(".select2-results__options li").each(function() {
				...
				if(!$this.attr("style")) {
					var paddingLeft =10+ (getLevel($this.attr("val"))) * 32;
					$("li[parent='" + $this.attr("parent") + "']").css("padding-left", paddingLeft + "px");
				}
				if($("li[parent='" + $this.attr("val") + "']").length==0) {
					var paddingLeft =10+ (getLevel($this.attr("val"))+1) * 32;
