...
;(function ($, window, document, undefined) {
	...
	Tree.prototype.unsubscribeEvents = function () {
		...
		this.$element.off('saveNode');
		this.$element.off('concelNode');
	};
	...
	Tree.prototype.subscribeEvents = function () {
		...
		if (typeof (this.options.onSaveNode) === 'function') {
			this.$element.on('saveNode', this.options.onSaveNode);
		}
	...
	*/
	Tree.prototype.setInitialStates = function (node, level) {
		...
		var parent = node;
        parent.children_total = parent.isParent?parent.nodes.length : 0;
		var _this = this;
		$.each(node.nodes, function checkStates(index, node) {

			// nodeId : unique, incremental identifier

			node.length = node.isParent?node.nodes.length : 0;
			//统计所有的孩子数量
            ...
            parent.children_total += node.children_total;
		});
	};
